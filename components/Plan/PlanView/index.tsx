...
export default function PlanView({ type }: Readonly<{ type: Plan['type'] }>): React.ReactElement {
    ...
    const incrementPageNumberDisabled = pageNumber >= plan.pageNumbers.max
    ...
    const handleFrameworkSelect = (value: string) => {
        setSelectedOptions((prev) => ({ ...prev, framework: value }))
    }

    React.useEffect(() => {
        if (plan.framework) setSelectedOptions((prev) => ({ ...prev, framework: plan.framework[0].name }))
    }, [plan.framework])

    React.useEffect(() => {
        setSelectedOptions((prev) => ({ ...prev, pageNumber, revision }))
    }, [pageNumber, revision])
                                ...
                                if (key === 'framework' && plan.framework)
                                    ...
                                    return (
                                        <div key={key} className='flex w-full items-center justify-start gap-x-2'>
                                            <button
                                                type='button'
                                                onClick={() => handleOptionSelect(key as keyof Plan['optionals'])}
                                                className={cn(
                                                    'cursor-pointer rounded-lg px-4 py-1 text-lg font-semibold transition-all duration-300',
                                                    selectedOptions[key as keyof Plan['optionals']].has
                                                        ? plan?.color?.addPrice
